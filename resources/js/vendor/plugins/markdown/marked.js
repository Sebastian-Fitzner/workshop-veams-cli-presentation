...
(function() {
	...
	block.html = replace(block.html)("comment", /\x3c!--[\s\S]*?--\x3e/)("closed",
		/<(tag)[\s\S]+?<\/\1>/)("closing", /<tag(?:"[^"]*"|'[^']*'|[^'">])*?>/)(/tag/g, block._tag)();
	...
	block.gfm.paragraph = replace(block.paragraph)("(?!", "(?!" + block.gfm.fences.source.replace("\\1",
		"\\2") + "|")();
	...
	Lexer.prototype.token = function(src, top) {
		...
		while (src) {
			...
			if (top && (cap = this.rules.table.exec(src))) {
				...
				for (i = 0; i < item.align.length; i++)
					if (/^ *-+: *$/.test(item.align[i])) item.align[i] = "right";
					else if (/^ *:-+: *$/.test(item.align[i])) item.align[i] =
					"center";
				else if (/^ *:-+ *$/.test(item.align[i])) item.align[i] = "left";
				else item.align[i] = null;
	...
	function InlineLexer(links, options) {
		...
		else if (this.options.pedantic) this.rules = inline.pedantic
	}
	...
	InlineLexer.prototype.outputLink = function(cap, link) {
		if (cap[0][0] !== "!") return '<a href="' + escape(link.href) + '"' + (link.title ? ' title="' + escape(link.title) + '"' : "") + ">" + this.output(cap[1]) + "</a>";
		else return '<img src="' + escape(link.href) + '" alt="' + escape(cap[1]) + '"' + (link.title ? ' title="' +
			escape(link.title) + '"' : "") + ">"
	};
	...
	Parser.prototype.tok = function() {
		switch (this.token.type) {
			...
			case "code":
				...
				if (!this.token.escaped) this.token.text =
					escape(this.token.text, true);
			...
			case "list_start":
				var type = this.token.ordered ? "ol" : "ul",
					body = "";
				while (this.next().type !== "list_end") body +=
					this.tok();
				return "<" + type + ">\n" + body + "</" + type + ">\n";
			...
			case "paragraph":
				return "<p>" + this.inline.output(this.token.text) +
					"</p>\n";
			case "text":
	...
	function merge(obj) {
		...
		for (; i < arguments.length; i++) {
			target = arguments[i];
			for (key in target)
				if (Object.prototype.hasOwnProperty.call(target, key)) obj[key] = target[key]
		}
		return obj
	}
	...
	function marked(src, opt, callback) {
		if (callback || typeof opt === "function") {
			...
			var tokens = Lexer.lex(tokens, opt),
				...
				l = tokens.length,
				i = 0;
			if (!highlight || highlight.length < 3) return callback(null, Parser.parse(tokens, opt));
			var done = function() {
	...
	marked.parse = marked;
	if (typeof exports === "object") module.exports = marked;
